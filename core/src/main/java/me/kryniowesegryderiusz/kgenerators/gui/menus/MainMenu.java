...
public class MainMenu {
	...
	public static Inventory get(Player player, int page)
	{
		...
		int lastId = -1;
		for (Entry<String, Generator> e : Main.getGenerators().getSpecifiedEntrySet(page*nrOfGenerators, nrOfGenerators))
		{
	...
	public static void onClick(Player p, int slot, ClickType clickType)
	{
		...
		else if (Main.getMenus().getMenuPlayer(p).getPage() > 0 && Lang.getMenuItemStorage().get(MenuItemType.MAIN_MENU_PAGE_PREVIOUS).getSlots().contains(slot) && Lang.getMenuItemStorage().get(MenuItemType.MAIN_MENU_PAGE_PREVIOUS).isEnabled())
		{
			Main.getMenus().openMainMenu(p, Main.getMenus().getMenuPlayer(p).getPage()-1);
		}
		else if (Main.getGenerators().getEntrySet().size() > (Main.getMenus().getMenuPlayer(p).getPage()+1)*Lang.getMenuItemStorage().get(MenuItemType.MAIN_MENU_GENERATOR).getSlots().size() 
		...
		{
			int lastId = -1;
			for (Entry<String, Generator> entry : Main.getGenerators().getSpecifiedEntrySet(Main.getMenus().getMenuPlayer(p).getPage()*Lang.getMenuItemStorage().get(MenuItemType.MAIN_MENU_GENERATOR).getSlots().size(), Lang.getMenuItemStorage().get(MenuItemType.MAIN_MENU_GENERATOR).getSlots().size()))
			{
				lastId++;
