...
public class IridiumSkyblockHook implements Listener {
	...
	@EventHandler
	public void onDeleteEvent (IslandRegenEvent e)
	{
		islandDeletion(e.getIsland());
	}
	...
	{
		World[] worlds = {IridiumSkyblockAPI.getInstance().getWorld(), IridiumSkyblockAPI.getInstance().getNetherWorld(), IridiumSkyblockAPI.getInstance().getEndWorld()};
		
		for (World w : worlds)
		{
			Location min = is.getPos1(w);
			Location max = is.getPos2(w);
			
			Logger.info("Detected IridiumSkyblock removing island in world " + min.getWorld().getName() + " starting at " + min.getBlockX() + "," + min.getBlockZ() + " and ending at " + max.getBlockX()+","+max.getBlockZ());
