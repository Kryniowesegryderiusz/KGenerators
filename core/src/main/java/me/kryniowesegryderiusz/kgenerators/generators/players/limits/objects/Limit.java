...
public class Limit {
		...
		if (config.contains(id + ".generators")) {
			...
			for (String s : gensIds) {
				...
				if (g != null) {
					...
					Logger.error(
							"Limits file: Cannot load generator " + s + " in " + id + ", because it doesnt exist!");
					error = true;
				}
			}
		} else {
	...
	public boolean fulfillsPlaceLimit(GeneratorPlayer gp, GeneratorLocation gloc, PlayerLimits pLimits) {
		...
		if (pLimits.getLimit(this) == -1 || getPlacedGenerators(gp) < pLimits.getLimit(this))
			...
			Lang.getMessageStorage().send(gp.getOnlinePlayer(), Message.GENERATORS_LIMITS_CANT_MORE, "<number>",
					String.valueOf(this.placeLimit), "<generator>",
					gloc.getGenerator().getGeneratorItem().getItemMeta().getDisplayName(), "<limit>", this.name);
			return false;
		}
	}
	...
	private boolean ownerCheck(GeneratorPlayer gp, GeneratorLocation gLocation, Message message) {
		...
		if (gp != gLocation.getOwner()) {
			Lang.getMessageStorage().send(player, message, "<owner>", gLocation.getOwner().getName());
			return false;
		}
	...
	public String toString() {
		...
		return String.format(
				"ID: %s | PlaceLimit: %s | OnlyOwnerUse: %s | OnlyOwnerPickup: %s | Affected generators: %s", this.id,
				this.placeLimit, this.onlyOwnerUse, this.onlyOwnerPickUp, generators);
	}
}
