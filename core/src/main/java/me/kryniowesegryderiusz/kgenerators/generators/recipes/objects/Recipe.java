...
public class Recipe {
	...
	public Recipe(RecipesManager recipesManager, Config file, String generatorID) {
		if (!generatorID.equals("example_generator") && !generatorID.equals("enabled")) {
			...
			if (generator == null) {
				Logger.error("Recipes file: There isnt any `" + generatorID
						+ "` generator in generators.yml! Recipe NOT LOADED!");
			} else {
				...
				for (String ingredientsString : ingredientsSection.getKeys(false)) {
					...
					ItemStack item = FilesUtils.loadItemStack(file, generatorID + ".ingredients", ingredientsString,
							false);

					shapedRecipe = Main.getMultiVersion().getRecipesLoader().setIngredient(shapedRecipe,
							ingredientsChar, item);
				...
				Logger.debugPluginLoad(
						"Recipes file: Loaded recipe for `" + generatorID + "`! Recipe: " + this.toString());
			}
		}
	}
