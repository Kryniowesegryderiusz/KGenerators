...
public class WorldGuardUtils_1_8 implements WorldGuardUtils {
	...
	WorldGuardPlugin worldGuard = (WorldGuardPlugin) Main.getInstance().getServer().getPluginManager()
			.getPlugin("WorldGuard");

	public static StateFlag PICK_UP_FLAG = new StateFlag(WGFlag.PICK_UP.getFlagId(), WGFlag.PICK_UP.getDefaultState());
	public static StateFlag ONLY_GEN_BREAK_FLAG = new StateFlag(WGFlag.ONLY_GEN_BREAK.getFlagId(),
			WGFlag.ONLY_GEN_BREAK.getDefaultState());
	public static StateFlag INTERACT_FLAG = new StateFlag(WGFlag.INTERACT.getFlagId(),
			WGFlag.INTERACT.getDefaultState());
			...
			for (WGFlag eflag : WGFlag.values()) {
				...
				try {
					...
					if (eflag == WGFlag.ONLY_GEN_BREAK)
						registry.register(ONLY_GEN_BREAK_FLAG);
